...
{	       
	...
	if(dd.obj)
	{
            var objeto = FindObj(dd.obj.name);
            if (objeto.tipoobj==10 || objeto.tipoobj==23 ) {
            try {
            if (objeto.clicar==1) {
            if(!arrastando) {
              arrastando=true;
              ...
              difArrX = dd.e.x - obj.x_ini;
              difArrY = dd.e.y - obj.y_ini;
		...
		if(dd.obj.nodrag) 
            {
              var objeto = FindObj(dd.obj.name);
              if (dd.obj.name.substring(0,5)=='Anima') {
		  if (objeto.tempos_3==0) {
                if (objeto.registra==1) {
    		  	window.setTimeout('Mensagem(\'Pense a Respeito\')', 50); 
	            eval('D'+dd.obj.name+'atual=0');
                } else if (objeto.registra==2) {
    		  	window.setTimeout('Mensagem(\'Muito Bem\')', 50); 
	            eval('D'+dd.obj.name+'atual=1');
		    }
 		    if (objeto.som!='') {
			var ss;
			ss = 's' + dd.obj.name;
                  if (somswf==0) {
                    toca_somNovo(objeto.som, false);
                  }
                  else if (somswf==2) {
                    tocaSomObj2(objeto.som);
                  }
                  else
                  {
                    tocaSomStreaming(objeto.som);
                  }
		    }
		    if (objeto.link!='' && objeto.funcao==0) {
			link_click(objeto.link, 1); 
		    } else if (objeto.linkprog!='') {
			window.open(objeto.linkprog, null, 'modal=yes, alwaysRaised=yes, scrollbars=1, resizable=1');
                } else if (objeto.funcao==-1) {
			volta();
                } else if (objeto.link!='' && objeto.funcao==1) {
			if (objeto.novajanela==0)
   			  link_click(objeto.link, '0', true); 
			else
			  link_click(objeto.link, '0', true, null, false); 
		    } else if (objeto.funcao==200) {
			eval('verifica=false');
			link_click('', 0, false, true);
		    } else if (objeto.funcao==200) {
			chamaEnvia();
		    } else if (objeto.funcao==309 || objeto.funcao==310) {
			chamaFinaliza();
		    }	else if (objeto.funcao==100) {
			window.close();
		    } else if (objeto.funcao>=311 && objeto.funcao<=316) {
			FuncoesFilme(objeto.funcao);
		    }
              } 
		  }
              dd.obj = null;
...
function DROP(d_ev)
{
      ...
      if(dd.obj)
	{
	    var objeto = FindObj(dd.obj.name);
          var obj_soltar;
          if (objeto.tipoobj==15 || objeto.tipoobj==16) { // liga
            colou = dragLigaCorreto(objeto, objeto.tipoobj, objeto.indice);
		if (colou==true) {
		  if (objeto.tipoobj==15) {
		    obj_soltar = FindObj('DLiga2' + objeto.indice);
                jg2.setColor(objeto.cor_linha);
		  } else {
		    obj_soltar = FindObj('DLiga1' + objeto.indice);
                jg2.setColor(obj_soltar.cor_linha); 
              } 

              if (obj_soltar.som!='') {
                  if (somswf==0) {
                    toca_somNovo(obj_soltar.som, false);
                  }
                  else if (somswf==2) {
                    tocaSomObj2(obj_soltar.som);
                  }
                  else
                  {
                    tocaSomStreaming(obj_soltar.som);
                  }
              }
              dd.obj.moveTo(objeto.x_ini, objeto.y_ini);  
                          	  	  
		  jg2.drawLine(objeto.x_ini+objeto.width, objeto.y_ini+objeto.height, obj_soltar.x_ini+obj_soltar.width, obj_soltar.y_ini+obj_soltar.height);
		  jg2.paint();
              obj_soltar = FindObj('DLiga2' + objeto.indice);
              obj_soltar.atual=1;
            } else {
          ...
          if (objeto.tipoobj==10 || objeto.tipoobj==23 || objeto.tipoobj==8) {
            if (objeto.tipoobj==8) 
            {
              colou = dragCorreto(objeto);
              obj_soltar = FindObj(objeto.soltado);  
            }
            else
            { 
            if (objeto.tipoobj==23) 
            {
              if (objeto.orfa==1) {
                objeto.erro=1;
                colou = false;
              } else {
                colou = dragCorretoArrN(objeto);
                obj_soltar = FindObj('DSoltarImgN' + objeto.indice_soltar);  
  		  } 
            }
            else 
            { 
              colou = dragCorretoImg(objeto);
              obj_soltar = FindObj('DSoltarImg' + objeto.indice_soltar);            
            ...
            if (colou==true) 
            { 
              objeto.atual = 1; // acerto
              arrastando=false; 
	        if (objeto.tipoobj==10) 
              {
                dd.obj.write('<img src="' + objeto.imagem + '" width="' + obj_soltar.width + '" height="' + obj_soltar.height + '">');
                dd.obj.moveTo(obj_soltar.x_ini, obj_soltar.y_ini);
    	 	    dd.obj.visible=false;
                if (obj_soltar.som!='') {
                  if (somswf==0) {
                    toca_somNovo(obj_soltar.som, false);
                  }
		    ...
		    { 
 //                 dd.obj.hide();
                  dd.obj.write('<img src="' + obj_soltar.img + '" width="' + obj_soltar.width + '" height="' + obj_soltar.height + '">');
...
function dragCorreto(objeto)
{
    var volta = false;
    var obj_soltar;
    for (i=1; i<= n_Arrastar; i++) {
      obj_soltar = FindObj('DSoltar' + i);
      if ((dd.obj.x>=obj_soltar.x_ini
           && dd.obj.x<=obj_soltar.x_ini + obj_soltar.width
           && dd.obj.y>=obj_soltar.y_ini
           && dd.obj.y<=obj_soltar.y_ini + obj_soltar.height) ||
      (dd.obj.x + objeto.width>=obj_soltar.x_ini
           && dd.obj.x + objeto.width<=obj_soltar.x_ini + obj_soltar.width
           && dd.obj.y>=obj_soltar.y_ini
           && dd.obj.y<=obj_soltar.y_ini + obj_soltar.height) ||
          (dd.obj.x<=obj_soltar.x_ini + obj_soltar.width
           && dd.obj.x>=obj_soltar.x_ini
           && dd.obj.y + objeto.height>=obj_soltar.y_ini
           && dd.obj.y + objeto.height<=obj_soltar.y_ini + obj_soltar.height) ||
      (dd.obj.x + objeto.width>=obj_soltar.x_ini
           && dd.obj.x + objeto.width<=obj_soltar.x_ini + obj_soltar.width
           && dd.obj.y + objeto.height>=obj_soltar.y_ini
           && dd.obj.y + objeto.height<=obj_soltar.y_ini + obj_soltar.height) ||
          (obj_soltar.x_ini >= dd.obj.x
           && obj_soltar.x_ini <= dd.obj.x + objeto.width
           && obj_soltar.y_ini >= dd.obj.y
           && obj_soltar.y_ini <= dd.obj.y + objeto.height) ||
      (obj_soltar.x_ini + obj_soltar.width >= dd.obj.x
           && obj_soltar.x_ini + obj_soltar.width <= dd.obj.x + objeto.width
           && obj_soltar.y_ini >= dd.obj.y
           && obj_soltar.y_ini <= dd.obj.y + objeto.height) ||
          (obj_soltar.x_ini <= dd.obj.x + objeto.width
           && obj_soltar.x_ini >= dd.obj.x
           && obj_soltar.y_ini + obj_soltar.height >= dd.obj.y
           && obj_soltar.y_ini + obj_soltar.height <= dd.obj.y + objeto.height) ||
      (obj_soltar.x_ini + obj_soltar.width>=dd.obj.x
           && obj_soltar.x_ini + obj_soltar.width <= dd.obj.x + objeto.width
           && obj_soltar.y_ini + obj_soltar.height >= dd.obj.y
           && obj_soltar.y_ini + obj_soltar.height <= dd.obj.y + objeto.height))
      {
        if (objeto.caption==obj_soltar.caption && obj_soltar.feito==0) {
          volta = true;
		  objeto.soltado = obj_soltar.id;
          obj_soltar.feito = 1;
	      obj_soltar.atual = 1;	
          dd.elements['DSoltarRot' + i].show();
          i = n_Arrastar + 1;
        }else{
...
function dragCorretoImg(objeto)
{
    var volta = false;
    var obj_soltar;
    for (i=1; i<= n_ArrImg; i++) {
      obj_soltar = FindObj('DSoltarImg' + i);
      if ((dd.e.x>=obj_soltar.x_ini
           && dd.e.x<=obj_soltar.x_ini + obj_soltar.width
           && dd.e.y>=obj_soltar.y_ini
           && dd.e.y<=obj_soltar.y_ini + obj_soltar.height))
      {
        if (objeto.indice_soltar==obj_soltar.indice_arrastar) {
          volta = true;   
		  obj_soltar.feito = 1;
        } else {
          vota = false;
...
function dragCorretoArrN(objeto)
{
    var volta = false;
    var obj_soltar;
    for (i=1; i<= n_SoltarN; i++) {
      obj_soltar = FindObj('DSoltarImgN' + i);
	  //obj_arr = FindObj('DArrastarImgN' + i);
      if ((dd.e.x>=obj_soltar.x_ini
           && dd.e.x<=obj_soltar.x_ini + obj_soltar.width
           && dd.e.y>=obj_soltar.y_ini
           && dd.e.y<=obj_soltar.y_ini + obj_soltar.height))
...
function dragLigaCorreto(objeto, tipoobj, indice)
{
    /*var volta = false;
    var obj_soltar;
    if (tipoobj==15)
      obj_soltar = FindObj('DLiga2' + indice);
    else
      obj_soltar = FindObj('DLiga1' + indice);
    if ((dd.obj.x>=obj_soltar.x_ini
           && dd.obj.x<=obj_soltar.x_ini + obj_soltar.width
           && dd.obj.y>=obj_soltar.y_ini
           && dd.obj.y<=obj_soltar.y_ini + obj_soltar.height) ||
